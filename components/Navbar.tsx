    ...
    >
      ...
      `}>
        ...
        <div className="hidden md:flex space-x-12 items-center">
          {NAV_LINKS.map((link) => (
            ...
            >
              {/* Render prop function to access isActive state dynamically */}
              {({ isActive }) => (
                <>
                  <span className={`transition-colors duration-300 ${isActive ? 'text-white' : 'text-gray-300 group-hover:text-white'}`}>
                    {link.name}
                  </span>
                  {/* Fluid Underline */}
                  <span
                    className={`absolute -bottom-1.5 left-0 h-[2px] bg-primary-brand transition-all duration-300 ease-out ${
          ...
          <Button 
            variant="outline" 
            to="/contact" 
            className="ml-8 rounded-full border-white/20 hover:bg-white/10 hover:border-white/40 px-6"
          >
            Contact &rarr;
          </Button>
        </div>
      ...
      <div 
        className={`
          ...
          ${isOpen ? 'max-h-[30rem] opacity-100 border-t border-white/10' : 'max-h-0 opacity-0'}
        `}
      >
        <div className="flex flex-col space-y-6 px-8 py-8">
          {NAV_LINKS.map((link) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`transition duration-300 ${isActive ? 'text-white' : 'text-gray-300 group-hover:text-white'}`}>
                    {link.name}
                  </span>
                  {/* Fluid Underline for Mobile */}
                  <span
                    className={`absolute -bottom-1 left-0 h-[2px] bg-primary-brand transition-all duration-300 ease-out ${
                      isActive ? 'w-full opacity-100' : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-100'
                    }`}
                  ></span>
                </>
          ...
          <div className="pt-4">
             <Button to="/contact" size="lg" className="w-full rounded-full border-white/20">
                Contact
            </Button>
          </div>
        </div>
